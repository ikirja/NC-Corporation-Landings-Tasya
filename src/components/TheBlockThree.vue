<template>
  <div class="content" id="about">
    <div class="block">
      <div class="block__item">
        <img class="block__item-header" src="@/assets/img/3_page_header.png" />
      </div>
      <div class="block__item">
        <div class="block__icon">
          <span class="block__icon-desc">Не оставит друга в беде</span>
          <img class="block__icon-img" src="@/assets/img/3_page_pic_1.png" />
        </div>
        <div class="block__icon">
          <span class="block__icon-desc">Умеет за себя постоять</span>
          <img class="block__icon-img" src="@/assets/img/3_page_pic_2.png" />
        </div>
        <div class="block__icon">
          <span class="block__icon-desc">Всегда придет на помощь</span>
          <img class="block__icon-img" src="@/assets/img/3_page_pic_3.png" />
        </div>
      </div>
      <div class="block__item">
        <div class="block__quote">
          <img class="block__quote-img" src="@/assets/img/3_page_Katya_face.png" />
          <p class="block__quote-desc">
            “Моя история повествует о приключениях удивительной и совершенно бесстрашной десятилетней девочки Таси, и посвящена бесценной детской дружбе и умению за себя постоять. На пути смелой и находчивой героини встречаются  сложные и, порой, совершенно неожиданные ситуации, требующие молниеносных решений, отваги и взаимопомощи, решений, от которых зависит благополучие и жизнь других.”
          </p>
        </div>
      </div>
    </div>
  </div>
</template>